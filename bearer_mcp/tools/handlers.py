...
from ..utils.path_utils import validate_path, resolve_scan_path
...
class ScanHandlerBase(ToolHandlerBase):
    ...
    def _build_scan_command(self, scan_path: Path, arguments: dict[str, Any]) -> list[str]:
    ...
    def _process_scan_result(self, result: dict[str, Any], output_format: str) -> list[types.TextContent]:
    ...
    def _process_successful_scan(self, result: dict[str, Any], output_format: str) -> list[types.TextContent]:
    ...
    def _process_text_output(self, output_text: str) -> list[types.TextContent]:
        ...
        return (
            ...
            if output_text
            else [self._create_text_content(
                "Bearer scan completed successfully. No security issues detected."
            )]
        )
    
    def _process_failed_scan(self, result: dict[str, Any], output_format: str) -> list[types.TextContent]:
...
class BearerVersionHandler(ToolHandlerBase):
    ...
    async def handle(self, arguments: dict[str, Any]) -> list[types.TextContent]:
        ...
        try:
            ...
            if result["success"]:
                return [self._create_text_content(f"Bearer CLI version:\n{result['stdout']}")]
            else:
                return [self._create_text_content(f"Failed to get Bearer version:\n{result['stderr']}")]
